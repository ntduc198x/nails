                  ...
                  {filteredRows.map((a) => {
                    ...
                    return (
                      <tr key={a.id} className="border-t border-neutral-100">
                        ...
                        <td>
                          {["BOOKED", "CHECKED_IN"].includes(a.status) ? <Link href={`/checkout?customer=${encodeURIComponent(customer)}&appointmentId=${a.id}`} className="btn btn-outline px-2 py-1 text-xs">Open ticket</Link> : <span className="text-xs text-neutral-400">Closed</span>}
                        </td>
                      </tr>
                    );
                  })}
